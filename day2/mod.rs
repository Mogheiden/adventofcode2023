...
const MAX_DICE: CubeCounts = CubeCounts {
    blue: 14,
    green: 13,
    red: 12,
};
    ...
    let output = input_str
        ...
        .filter_map(|line| {
            ...
            let is_game_valid = sets_str.split("; ").all(|set_str| {
                ...
                let is_set_valid = cubes.blue <= MAX_DICE.blue
        ...
        .map(|line| {
            ...
            sets_str.split("; ").for_each(|set_str| {
                for cube_count_str in set_str.split(", ") {
                    ...
                    match color {
                        "red" => {
                            if min_cubes.red < amount {
                                min_cubes.red = amount
                            }
                        }
                        "green" => {
                            if min_cubes.green < amount {
                                min_cubes.green = amount
                            }
                        }
                        "blue" => {
                            if min_cubes.blue < amount {
